...
<script type="text/Javascript">
	...
	function mainValues() {
		var value1 = document.getElementById('frmList').province_name.value.trim();
		var value2 = document.getElementById('frmList').province_code.value.trim();
		parent.PopupModal('', '', 'hide');
		parent.PushChild('KODE_PROVINSI_FRONT', 'Registration', value1, value2);
	}

	function getValue(e1,e2) {
		document.getElementById('frmList').province_name.value = e2;
		document.getElementById('frmList').province_code.value = e1;
		mainValues();
	}
	...
	<body class="container">
		<form id="frmList" method="post">
			<div class="form-wrap" id="form-wrap">
					<div class="dataTable_wrapper table">
                    	<table class="table table-striped table-bordered table-hover" id="ListBank">
		                    <thead>
		                        <tr>
			        				<th><?= $this->lang->line('province_code') ?></th>
			        				<th><?= $this->lang->line('province_name') ?></th>
			        				<th></th>	        				
					    		</tr>
							</thead>
		                    <tbody></tbody>
			...
			<input type="hidden" value="" name="province_code" id="province_code" />
    ...
    $(document).ready(function() {
       ...
       $('#ListBank').DataTable({
            ...
            "searching": true,
            "ajax"   : {
                url : "<?=base_url();?>Form.jsp/FrontPopup.jsp/show_kode_provinsi_db",
                type: "post",
                data: postData,
            },
