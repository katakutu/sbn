...
<META HTTP-EQUIV="Expires" CONTENT="-1">
	...
	jQuery(function($) {
		...
		function filterInvestor(){
			...
			$.ajax({
		        url : "<?=base_url()?>Pemesanan.jsp/show_order_filter",
		        type: "post",
		        dataType: "JSON",
		        ...
		        success: function(data) {
		        	...
		        	var b=document.forms["filter"]["sid2"].value;
		        	var c=document.forms["filter"]["seriid"].value;
		        	var d=document.forms["filter"]["ordercode"].value;
		        	var e=document.forms["filter"]["billcode"].value;
		        	if(a==null || a=="",b==null || b=="",c==null || c=="",d==null || d=="",e==null || e=="",((a==null || a=="") && ((b==null || b=="") || (c==null || c=="")) && (d==null || d=="") && (e==null || e=="")))
		        	{
		        	...
		        	else if(b&&c)
		        	{
		        		...
		        		if(data[0].hasOwnProperty('KodePemesanan'))
			        	{
		...
		function checkSid(){
		    ...
		    document.getElementById('seriname').disabled = this.checked;
		    document.getElementById('ordercode').disabled = this.checked;
		    document.getElementById('billcode').disabled = this.checked;
		    $('#ordercode,#billcode,#sid2,#seriname,#seriid').val('');
			};
		}
		...
		function checkOrder(){
		    ...
		    document.getElementById('ordercode').disabled = !this.checked;
		    document.getElementById('billcode').disabled = this.checked;
		    $('#sid,#sid2,#seriname,#billcode,#seriid').val('');
			};
		}
		...
		function checkBill(){
		    ...
		    document.getElementById('ordercode').disabled = this.checked;
		    document.getElementById('billcode').disabled = !this.checked;
		    $('#sid,#sid2,#seriname,#ordercode,#seriid').val('');
			};
		}
		...
		function checkSidSeri(){
		    ...
		    document.getElementById('billcode').disabled = this.checked;
		    document.getElementById('sid2').disabled = !this.checked;
		    $('#sid,#seriname,#ordercode,#billcode,#seriid').val('');
			};
		}
	...
	<body oncontextmenu="return false">
		<div class="panel panel-primary">
			...
			<div class="panel-body" id="invesdetail">
				...
				<span class="input-group-addon" id="addon-sid"><input type="radio" name="check" id="checker" onclick="checkSid()">&nbsp;<?= $this->lang->line('sid_subreg') ?>&nbsp;</span>
				<input class="form-control" type="text" placeholder="<?= $this->lang->line('sid_subreg') ?>" aria-describedby="addon-sid" id="sid" name="sid" maxlength="70" required disabled onkeypress="return briIbbiz.isAlphaNum(event)"/>
				</div>
				<div>
					<span id="SPAN_sid" style="color:red; font-size:11px"><?=form_error('sid_subreg',' ',' ')?></span>
				</div>
				<br />
				<div class="input-group input-group">
					...
					<input class="form-control" type="text" placeholder="<?= $this->lang->line('seriname') ?>" aria-describedby="addon-seriname" id="seriname" name="seriname" maxlength="70" required readonly/>
				...
				<br />
				<div class="input-group input-group">
					<span class="input-group-addon" id="addon-ordercode"><input type="radio" name="check" id="checking" onclick="checkOrder()"> <?= $this->lang->line('ordercode') ?>&nbsp;</span>
					<input class="form-control" type="text" placeholder="<?= $this->lang->line('ordercode') ?>" aria-describedby="addon-ordercode" id="ordercode" name="ordercode" maxlength="70" required disabled onkeypress="return briIbbiz.isNumberKey(event)"/>
				</div>
				<div>
					<span id="SPAN_ordercode" style="color:red; font-size:11px"><?=form_error('ordercode',' ',' ')?></span>
				</div>
				<br />

				<div class="input-group input-group">
					<span class="input-group-addon" id="addon-billcode"><input type="radio" name="check" id="check" onclick="checkBill()"> <?= $this->lang->line('billcode') ?>&nbsp;</span>
					<input class="form-control" type="text" placeholder="<?= $this->lang->line('billcode') ?>" aria-describedby="addon-billcode" id="billcode" name="billcode" maxlength="70" required disabled onkeypress="return briIbbiz.isNumberKey(event)"/>
				</div>
				<div>
					<span id="SPAN_billcode" style="color:red; font-size:11px"><?=form_error('billcode',' ',' ')?></span>
				</div>
				<br />
				...
				<div class="panel-body" id="form-wrap">
					<div class="dataTable_wrapper table">
		            	<table class="table table-striped table-bordered table-hover" id="ListInvestor" cellspacing="0" width="100%">
		                    ...
		                    <tbody id="tbodyinves" style="text-align: center;"></tbody>
		                </table>
					</div>
				</div>
				</div>
	...
	<!--<link href="<?php echo base_url()."plugin/datatables/datatables/media/css/jquery.dataTables.css";?>" rel="stylesheet" type="text/css" />-->
