...
<script type="text/Javascript">
	...
	function mainValues() {
		var value1 = document.getElementById('frmList').subregname.value.trim();
		var value2 = document.getElementById('frmList').subregid.value.trim();
		parent.PopupModal('', '', 'hide');
		parent.PushChild('KODE_SUBREG', 'Investor', value1, value2);
	}

	function getValue(e1,e2) {
		document.getElementById('frmList').subregname.value = e2;
		document.getElementById('frmList').subregid.value = e1;
		mainValues();
	}
	...
	<body class="container">
		<form id="frmList" method="post">
                    	...
                    	<table class="table table-striped table-bordered table-hover" id="ListBank">
		                    <thead>
		                        <tr>
			        				...
			        				<th><?= $this->lang->line('subregname') ?></th>
			        				<th></th>        				
					    		</tr>
							</thead>
		                    <tbody></tbody>
			...
			<input type="hidden" value="" name="subregname" id="subregname" />
			<input type="hidden" value="" name="subregid" id="subregid" />
    ...
    $(document).ready(function() {
        ...
        $.ajax({
	        type: 'POST',
	        url: '<?=base_url();?>ListKdSubreg.jsp/show_kode_subreg',
	        data: {},
	        dataType: 'JSON',
	        dataSrc: '',
	        beforeSend: function() {
