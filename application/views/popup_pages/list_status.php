...
<script type="text/Javascript">
	...
	function mainValues() {
		...
		var value1 = document.getElementById('frmList').statid.value.trim();
		var value2 = document.getElementById('frmList').stats.value.trim();

		parent.PopupModal('', '', 'hide');
		parent.PushChild('STATUS', 'filter', value1, value2);
	}

	function getValue(e1,e2) {
		
		document.getElementById('frmList').statid.value = e1;
		document.getElementById('frmList').stats.value = e2;
		mainValues();
	}
		...
		<form id="frmList" method="post">
			<div class="form-wrap" id="form-wrap">
					<div class="dataTable_wrapper table">
                    	<table class="table table-striped table-bordered table-hover" id="ListStatus">
		                    <thead>
		                        <tr>
			        				<th><?= $this->lang->line('id status') ?></th>
			...
			<input type="hidden" value="" name="statid" id="statid" />
			<input type="hidden" value="" name="stats" id="stats" />
    ...
    $(document).ready(function() {
        ...
        $.ajax({
	        type: 'POST',
	        url: '<?=base_url();?>ListSeriOffer.jsp/show_status',
	        data: {},
	        dataType: 'JSON',
	        dataSrc: '',
	        beforeSend: function() {
        ...
        function drawTableListAdmin(response){
	        var myTable = $('#ListStatus').DataTable({
	        "aaData": response,
	        "columns": [
		        		  { "data": "Id" },
			        	  { "data": "Nama" },
			...
			$('#ListStatus tbody').on( 'click', 'a', function () {
